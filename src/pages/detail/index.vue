<template>
  <view class="main">
    <swiper :autoplay="true" interval="3000" class="swiper">
      <block v-for="(item,index) in detail.bgs" :key="index">
        <swiper-item>
    ...
    <view class="info">
      <view class="left">
        ...
        <view class="tags">
          <text v-for="(item,index) in detail.tags" :key="index">{{item}}</text>
        </view>
        <view class="ability">
          <view class="row">
...
<script>
const list = require("../../../static/data/lol_details_duowan");
const app = getApp();
export default {
  data() {
    return {
  ...
  mounted() {
    const res = list.filter(item => {
      return item.id === this.$root.$mp.query.id;
    });
    this.detail = res[0];
    ...
    wx.getSystemInfo({
      success: res => {
        this.windowHeight = res.windowHeight / (res.windowWidth / 750);
        this.screenHeight = res.screenHeight / (res.screenWidth / 750);
      }
    });
...
.bar {
  ...
  border-radius: 16px;
  box-shadow: 0px 0px 12px 0px rgba(126, 234, 25, 1),
...
.bar.color2 {
  ...
  background: linear-gradient(
    to bottom,
    rgba(229, 195, 25, 1) 0%,
    rgba(168, 140, 0, 1) 100%
  );
  box-shadow: 0px 0px 12px 0px rgba(229, 195, 25, 1),
    inset 0px 1px 0px 0px rgba(255, 255, 255, 0.45),
    inset 1px 0px 0px 0px rgba(255, 255, 255, 0.25),
    inset -1px 0px 0px 0px rgba(255, 255, 255, 0.25);
}
